...
\begin{frame}[shrink=5]{Feature Engineering: 33 Columns $\rightarrow$ 73 Features}
  \begin{columns}[T]
    ...
    \end{tabular}
    \vspace{0.2cm}
    \begin{block}{\small Final feature matrix}
      \small\textbf{37 features} (union top-30 Lasso + top-20 RF)\\
      on 63\,520 listings
    \end{block}
...
\begin{frame}{All 8 Models --- Performance Comparison}
  \begin{columns}[T]
    ...
    \includegraphics[width=\linewidth]{model_comparison_full.png}
  \end{columns}
\end{frame}

\begin{frame}[shrink=18]{R\textsuperscript{2} with Cross-Validation Uncertainty}
  \begin{columns}[T]
    \column{0.50\textwidth}
    ...
    \end{itemize}
    \vspace{0.25cm}
    \includegraphics[width=\linewidth]{gridsearch_heatmap.png}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=3]{Amenity Analysis: Which Amenities Add Value?}
  \begin{columns}[T]
    ...
    \begin{block}{Actionable insight}
      \small Adding amenities with high premium \% to your listing
